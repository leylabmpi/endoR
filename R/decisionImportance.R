...
decisionImportance <- function(rules, data, target, classPos = NULL
  ...
  if (in_parallel == TRUE){
    ...
    tmp <- parApply(cl = cluster, rules, MARGIN = 1, FUN = importanceSingleRule
      , data = data, target = target, type = type, pred_null = pred_null)

  } else {
    tmp <- apply(rules, MARGIN = 1, FUN = importanceSingleRule
      , data = data, target = target, type = type, pred_null = pred_null)
  }
...
function(rule,data,target,type='reg', pred_null = NULL){
  ...
  if (type=='reg') {
    ...
    err_null <- exp(mean( sapply(ys, function(x,pred){x*log(pred)+(1-x)*log(1-pred)}, pred = pred_null) ))
    err_null <- 1 - err_null
  }
