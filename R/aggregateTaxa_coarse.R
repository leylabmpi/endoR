aggregateTaxa_coarse <- function(taxa, weights, thr = NULL){
    ...
    if (is.null(thr) == TRUE){thr <- mean(weights$weight)}

    #some formatting
    if (!('data.table' %in% class(taxa))){taxa <- as.data.table(taxa)[,Feature:=as.character(Feature)]}
    ...
    for (i in 1:lim){
        ...
        if (length(to_check)>0){
            ...
            if (length(better) >0){
                ...
                taxa <- taxa[ix %in% to_change, `:=`(newFeature = taxa[['newFeature']][max(better)]
