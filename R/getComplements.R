getComplements <- function(rules, data, target, classPos = NULL,
  ...
  directions <- data.table(
    ruleID = str_extract(names(sVar), pattern = "^decision[:digit:]+(?=var)"),
  ...
  tmp <- unique(copy(rulesRm)[condition != "", list(condition)]) # remove repetitions due to the dummy variables
  if (nrow(tmp) == 0) {
    message("There is no interaction in the decision ensemble, only single variable effects.")
  } else {
    suppressMessages(
      tmp <- getDecisionsMetrics(tmp,
...
complementSingleRule <- function(rule) {
  ...
  if (length(in_dup) > 0) {
    for (i in in_dup) {
      ruleExec[i - 1] <- paste(ruleExec[i - 1], ruleExec[i], sep = " & ")
    }
