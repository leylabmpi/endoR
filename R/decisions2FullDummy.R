...
decisions2FullDummy <- function(rules, data, in_parallel = FALSE, n_cores = detectCores() - 1, cluster = NULL) {
  ...
  if (length(notNum) == 0) {
    return(list("rules" = as.data.table(rules), "data_dummy" = data))
  }
  ...
  if ("condition" %in% rownames(rules)) {
    rules <- t(rules)
  }
  ...
  if (in_parallel == TRUE && nrow(rules) > n_cores * 5) {
    ...
    rules[, "condition"] <- unlist(parLapply(
      cl = cluster, tmp,
      fun = function(X, patVector) {
        str_replace_all(X, patVector)
      },
      patVector = mapCol
    ))
  } else {
