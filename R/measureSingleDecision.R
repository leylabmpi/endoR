...
function(ruleExec,data,target,type = 'reg', gain = FALSE, pred_null = NULL){
  ...
  } else {colNames <- c("len","support","err","condition","pred", "gain") }
  ...
  if (type == 'reg') {
    ...
    if (gain == FALSE){
      ...
      err_null <- 1 - exp(mean( sapply(ys, function(x,pred){x*log(pred)+(1-x)*log(1-pred)}, pred = pred_null) ))
      g <- 1 - err / err_null
      v <- c(len, support, err, origRule, pred, g)
    }
  }
...
function(data,target, classPos = NULL){
  ...
  if (!is.numeric(target)){
    ...
    err <- 1 - exp(mean( sapply(target, function(x,pred){x*log(pred)+(1-x)*log(1-pred)}, pred = pred) ))
  } else {
    pred <- mean(target, na.rm = TRUE)
