...
stabilitySelection <- function(rules, alpha_error = 1, pi_thr = 0.7, aggregate_taxa = FALSE, taxa = NULL) {
  ...
  pall <- mean(sapply(rules, function(x) {
    return(x$pdecisions)
  }))
  qsubset <- sqrt((2 * pi_thr - 1) * alpha_error * pall)
  if (qsubset < 1) {
    qsubset <- 1
  } # if less than a decision should be selected, take at least one
  ...
  rules_agg <- lapply(rules, function(x, qsubset) {
    setorder(x$rules[, wimp := imp * n], -wimp)[, wimp := NULL]
  ...
  # get all edges
  edges_agg <- lapply(rules, function(x) {
    return(x$edges_agg)
  })
  ...
  # get all nodes
  nodes_agg <- lapply(rules, function(x) {
    return(x$nodes_agg)
  })
